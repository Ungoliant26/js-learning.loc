...
const meanScore = (...midArray) => {
  // ... rest оператор
  if (midArray.some((mid) => typeof mid !== 'number')) {
    console.error('Все аргументы в вызове функции должны быть числами!')
    return
  }

  /**
   * userNumber1 / midArray.length + userNumber2 / midArray.length + ... + userNumber10 / midArray.length
   * Равносильно
   * userNumber1 + userNumber2 + ... + userNumber10 / midArray.length
   */
  return midArray
    .reduce((mid, userNumber) => mid + userNumber / midArray.length, 0)
    .toFixed(2)
}
...
console.log(meanScore(...scores1)) // ... spread оператор
// 1.93
